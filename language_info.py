def create_node(node, variable_name, used_vars: dict, var_node_mapping: dict, triples: list, category: str, replace=False):
    """
    ...
    new_var_name = variable_name(category, used_vars, var_node_mapping)

    if replace:
        old_var_name = next((k for k, v in var_node_mapping.items() if v == node), None)  # ragionare su questo None
...
def possessives(node, var_node_mapping: dict, used_vars: dict, triples: list, variable_name, artificial_nodes, find_parent, role=None):
    """
    ...
    if node.feats['PronType'] == 'Prs':
        var_name, var_node_mapping, triples = create_node(node, variable_name, used_vars, var_node_mapping, triples, 'person', replace=True)
        try:
...
def quantifiers(node, var_node_mapping: dict, used_vars: dict, triples: list, variable_name, add_node, artificial_nodes, find_parent, role=None):
    """
    Function to handle quantifiers (e.g., omnis).
    """
    if node.feats['PronType'] == 'Tot':
        if node.parent.upos in ['ADJ', 'NOUN', 'PROPN']:
            add_node(node, var_node_mapping, triples, artificial_nodes, find_parent, role)
        elif node.parent.upos == 'VERB':
            type_arg = 'thing' if node.feats['Gender'] == 'Neut' else 'FILL'
            var_name, var_node_mapping, triples = create_node(node, variable_name, used_vars, var_node_mapping, triples, type_arg)
            parent = find_parent(node, var_node_mapping, artificial_nodes)
            triples.append((parent, role, var_name))
            # attaching the quantifier itself
            add_node(node, var_node_mapping, triples, artificial_nodes, find_parent, role='quant', def_parent=var_name)

    return triples


def det_pro_noun(node, var_node_mapping: dict, used_vars: dict, triples: list, variable_name, artificial_nodes, find_parent, role=None):
    """
    ...
    if node.deprel != 'det' and node.feats['PronType'] == 'Dem':
        ...
        var_name, var_node_mapping, triples = create_node(node, variable_name, used_vars, var_node_mapping, triples, type_arg, replace=True)
        parent = find_parent(node, var_node_mapping, artificial_nodes)
        triples.append((parent, role, var_name))
    return triples